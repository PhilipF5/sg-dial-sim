import { AddressGlyphDisplayComponent } from "./address-glyph-display/address-glyph-display.component";
import { AddressRowComponent } from "./address-row/address-row.component";
import { AddressSetsListComponent } from "./address-sets-list/address-sets-list.component";
import { ChevronBoxComponent } from "./chevron-box/chevron-box.component";
import { ChevronStatusComponent } from "./chevron-status/chevron-status.component";
import { DialingStatusComponent } from "./dialing-status/dialing-status.component";
import { FloatingChevronBoxComponent } from "./floating-chevron-box/floating-chevron-box.component";
import { GlyphEntryComponent } from "./glyph-entry/glyph-entry.component";
import { KeyboardComponent } from "./keyboard/keyboard.component";

export { AddressGlyphDisplayComponent } from "./address-glyph-display/address-glyph-display.component";
export { AddressRowComponent } from "./address-row/address-row.component";
export { AddressSetsListComponent } from "./address-sets-list/address-sets-list.component";
export { ChevronBoxComponent } from "./chevron-box/chevron-box.component";
export { ChevronStatusComponent } from "./chevron-status/chevron-status.component";
export { DialingStatusComponent } from "./dialing-status/dialing-status.component";
export { FloatingChevronBoxComponent } from "./floating-chevron-box/floating-chevron-box.component";
export { GlyphEntryComponent } from "./glyph-entry/glyph-entry.component";
export { KeyboardComponent } from "./keyboard/keyboard.component";

export const COMPONENTS = [
	AddressGlyphDisplayComponent,
	AddressRowComponent,
	AddressSetsListComponent,
	ChevronBoxComponent,
	ChevronStatusComponent,
	DialingStatusComponent,
	FloatingChevronBoxComponent,
	GlyphEntryComponent,
	KeyboardComponent,
];
